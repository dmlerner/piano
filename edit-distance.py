...
def get_distance_dp(a, b):
    ...
    for i in range(1, len(a)+1):
        for j in range(1, len(b)+1):
            ...
            dp[i][j] = get_minimum_and_argmin(options)
...
def test_get_note_uses():
    ...
    I = range(12)
    scale = [Note(i, i, 1) for i in I]
    played = [Note(i, i-.05+.1*random.random(), .95+.1*random.random()) for i in I]
    mistaken_note = Note(100, 4.3, 300)
    played.append(mistaken_note)
    played.insert(0, Note(100, -54, 300))
...
def test_algorithms():
    Note.note_distance_weight = 1
    Note.start_distance_weight = 1
    Note.duration_distance_weight = 1

    Note.deletion_weight = 1 + 2**-3
...
def parse(filename):
    ...
    for line in lines:
        ...
        notes.append(Note(note, start, 1))
    return notes
